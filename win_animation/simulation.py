...
class Simulation:
    ...
    def _create_composite_bg(self):
        ...
        # 叠加图片（如果有），贴齐窗口左上角
        if self.bg_image:
            composite.blit(self.bg_image, (0, 0))
    ...
    def draw_menu(self, mouse, scroll, size=50, font=pygame.font.SysFont("Consolas", 20), colour=(255, 253, 219)):
        """绘制菜单以编辑粒子类型/颜色之间的吸引值，确保内容清晰可见"""
        text = font.render(f"FPS: {int(clock.get_fps())}", True, colour)  # 显示程序的 FPS
        ...
        for i in range(n):
            for j in range(n):
                ...
                # 使用更大的字体和高对比度颜色确保数字清晰可见
                text = font.render(f"{round(self.attraction_matrix[i][j], 1)}", True, (0, 0, 0))  # 黑色字体
                rect = text.get_rect(center=box.center)  # 将文本居中
                screen.blit(text, rect)  # 绘制文本

                if box.collidepoint(mouse[0], mouse[1]):  # 如果鼠标悬停在一个框上
                    self.attraction_matrix[i][j] += scroll  # 根据滚动方向调整值
                    self.attraction_matrix[i][j] = min(1, max(-1, self.attraction_matrix[i][j]))  # 将值限制在 -1 到 1 之间

                if i == 0:  # 顶行的圆圈
                    pygame.draw.circle(screen, colours[j], (box.left + size / 2, box.top - size / 2), config.radius * 2)
                    pygame.draw.circle(screen, colour, (box.left + size / 2, box.top - size / 2), config.radius * 2, 1)  # 外框
                if j == 0:  # 左列的圆圈
                    pygame.draw.circle(screen, colours[i], (box.left - size / 2, box.top + size / 2), config.radius * 2)
                    pygame.draw.circle(screen, colour, (box.left - size / 2, box.top + size / 2), config.radius * 2, 1)  # 外框

        rect = pygame.Rect(WIDTH / 2 - (n * size) / 2, HEIGHT / 2 - (n * size) / 2 + n * size, size * n, size)  # 矩阵下方的框
        text = font.render("Random", True, colour)
        rect = text.get_rect(center=rect.center)  # 将文本居中
        if rect.collidepoint(mouse[0], mouse[1]) and scroll:
            self.attraction_matrix = np.random.uniform(-1, 1, (len(colours), len(colours)))  # 如果滚动时悬停在此框上，随机设置吸引值
