...
class Button:
    def __init__(self, button_title, position, width=BTN_WIDTH,
        ...
        self.font = pygame.font.Font('game/fonts/MATURASC.TTF', FONT_SIZE)
...
class UiManager:
    def __init__(self, screen, level):
        ...
        self.win_label = Label('You won the game!', (WIDTH // 2, HEIGHT // 2 -
                                               2 * BTN_HEIGHT))
        self.win_game_display = Display(buttons=[self.start_game_again_btn,
                                                 self.finish_btn],
                                        labels=[self.win_label])

        self.new_game_button = Button('New Game', SCREEN_CENTER,
                                      background_color=TAUPE,
                                      font_color=BROWN)
